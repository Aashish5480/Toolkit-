...
import { CHEMISTRY_FORMULAS } from "../../data/scienceData";
import { Search, FlaskConical, Copy, Check, Filter, Sparkles, BookOpen } from "lucide-react";

export function ChemistryFormulaFinder() {
  ...
  const categories = [
    "All Categories",
    ...Array.from(new Set(CHEMISTRY_FORMULAS.map((f) => f.category)))
  ];

  const filtered = CHEMISTRY_FORMULAS.filter((item) => {
    ...
    <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs p-5 sm:p-7 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-100">
        <div>
          <h2 className="text-base font-bold text-slate-900">
            NCERT Chemistry Formula & Reaction Directory
          </h2>
          <p className="text-xs text-slate-500">
            Comprehensive formula sheet with SI units, variable legends, and numerical examples
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
        <div className="sm:col-span-8 relative">
          ...
          <input
            ...
            placeholder="Search chemistry formula (e.g. Nernst, Raoult, Arrhenius, Molarity)..."
      ...
      <div className="space-y-4">
        <div className="text-xs font-bold text-slate-500 uppercase tracking-wider">
          Found {filtered.length} Formulas & Equations
        </div>
        ...
        {filtered.length > 0 ? (
              ...
              >
                <div>
                  <div className="flex items-center justify-between gap-2 flex-wrap mb-1">
                    ...
                    <button
                      onClick={() => handleCopy(`chem-${idx}`, `${item.name}: ${item.formula}`)}
                ...
                {item.exampleProblem && (
                  <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs space-y-1 text-slate-700">
                    <span className="font-bold text-slate-900 block">Board Exam Numerical:</span>
          ...
          <div className="p-8 text-center bg-slate-50 rounded-xl text-slate-500 text-sm">
            No formulas found matching "{searchQuery}". Try searching for 'Nernst' or 'Raoult'.
          </div>
        )}
      </div>
    </div>
